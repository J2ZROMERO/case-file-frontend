import { Building2, CalendarDays, ClipboardList, FileText, LogOut, ShieldCheck, Stethoscope, UserCog, Users } from "lucide-react";
...
const navItems = [
  ...
  { id: "clinics", label: "Clínicas", icon: Building2, roles: ["tenant_admin"] },
  { id: "staff", label: "Personal", icon: UserCog, roles: ["tenant_admin"] },
  { id: "audit", label: "Actividad", icon: ClipboardList, roles: ["tenant_admin", "auditor"] },
];
        ...
        <aside className="min-w-0 border-b border-slate-200 bg-white lg:border-b-0 lg:border-r">
          <div className="flex items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:block lg:p-5">
            <div>
              ...
              <h1 className="text-xl font-bold text-ink sm:text-2xl">Expediente Clínico</h1>
            </div>
            {isAuthenticated ? (
              <button
                type="button"
          ...
          <nav
            className="fixed inset-x-0 bottom-0 z-40 flex overflow-x-auto border-t border-slate-200 bg-white/95 px-2 pb-[max(.5rem,env(safe-area-inset-bottom))] pt-2 shadow-[0_-8px_24px_rgba(15,23,42,.08)] backdrop-blur lg:static lg:grid lg:grid-cols-1 lg:gap-2 lg:overflow-visible lg:border-0 lg:bg-transparent lg:px-4 lg:pb-4 lg:pt-0 lg:shadow-none"
          >
            {visibleItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-20 flex-none flex-col items-center justify-center gap-1 rounded-lg px-1 py-2 text-[11px] font-semibold transition lg:w-auto lg:flex-row lg:justify-start lg:gap-2 lg:px-3 lg:text-sm ${
                    activeView === item.id
